...
/* Includes ---------------------------------------------------------------- */
#include <Plant_ai_inferencing.h>
...
#else
        ei_printf("\r\n\r\n");
        uint8_t hoogsteIndex = 0;
        float hoogsteWaarde = 0;
        for (uint16_t i = 0; i < EI_CLASSIFIER_LABEL_COUNT; i++) {
            if(result.classification[i].value > hoogsteWaarde){
              hoogsteWaarde = result.classification[i].value;
              hoogsteIndex = i;
            }
        }
        ei_printf("Volgens de ai is deze foto een: %s\r\n", ei_classifier_inferencing_categories[hoogsteIndex]);
        ei_printf("met een probabiliteit van %f\r\n", hoogsteWaarde);
#endif
